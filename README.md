...
> 🏆 **Top 12 Finalist at HackOut'26 (@ DA-IICT)**  
> **Team Name:** VZEYA | **Team Leader:** Harsh Solanki | **Team Members:** Harsh Solanki (Backend), Panchal Lakshya (Frontend), Ubaid Khan (AI)  
> **Demo / Project Name:** ABC Bank (Adaptive Banking for Bharat)

> *"Instead of asking customers to learn their bank, make the bank learn the customer."*

ABC Bank is an intelligence and experience layer that dynamically adapts a customer's mobile banking interface based on transaction history, cash-flow pulse, life-stage signals, risk indicators, and interaction preferences — designed for Tier 1 to Tier 4, rural, and vernacular-first users across Bharat.

---

## 🏆 Hackathon Recognition — Top 12 Selected Teams

Team **VZEYA** (Project: **ABC Bank**) was selected among the **Top 12 Finalist Teams** at **HackOut'26 (DA-IICT)**:

📄 **[Download Official Shortlist PDF](docs/assets/HackOut26_Top12_Shortlist.pdf)**

| # | Team Name | Team Leader Name | Status |
|---|---|---|---|
| 1 | Zero Sum | Patel Vedant | Selected Finalist |
| 2 | Wowwsters | Kashyap Vadher | Selected Finalist |
| 3 | Outliers | Jeevesh Bodhani | Selected Finalist |
| 4 | Kinzz | Kinjal Rantanpara | Selected Finalist |
| 5 | Foxbat | Viraj Patel | Selected Finalist |
| 6 | ByteMe | Tanmay Kanani | Selected Finalist |
| 7 | Tech Titans | Dhruvil Prajapati | Selected Finalist |
| 8 | Tech Titans | Savaliya Saumil | Selected Finalist |
| 9 | Tatva | Shaurya Doshi | Selected Finalist |
| 10 | cosmiX | Vedant Bhatt | Selected Finalist |
| 11 | Ctrl Freaks | Aum Pethani | Selected Finalist |
| 12 | **VZEYA** *(Our Team)* | **Harsh Solanki** | **Selected Finalist (Demo: ABC Bank)** |

---

## Table of Contents

- [🏆 Hackathon Recognition — Top 12 Selected Teams](#-hackathon-recognition--top-12-selected-teams)
...
Traditional banking apps present the same static interface to every customer regardless of their financial situation. A customer in financial distress sees the same loan offers as a thriving saver. A daily metro commuter gets the same home screen as a rural farmer. ABC Bank solves this by:
...
flowchart TD
  subgraph Users["End Users"]
    ...
    U2["Rural and Vernacular-First Users"]
    U3["Metro Commuters"]
  end
  ...
  subgraph MobileApp["React Native / Expo App (apps/frontend/)"]
    ...
    subgraph Screens["Feature Screens"]
      HomeScreen["AdaptiveHomeScreen<br/>(Dynamic Hero + Context Cards)"]
      ChatScreen["MitraChatScreen<br/>(Vernacular AI Chat)"]
      PayScreen["PaymentsScreen<br/>(Transfer + 1-Tap Pay)"]
      TxnScreen["TransactionsScreen<br/>(Combined Ledger)"]
      InsightScreen["InsightsScreen<br/>(Spending Analysis)"]
      ProfileScreen["ProfileScreen<br/>(Settings + KYC)"]
    end
    
    subgraph ClientServices["Client Services"]
      ZustandStore["Zustand Store<br/>(useCustomerStore)"]
      BankingAPI["BankingApi<br/>(fetch + 2.5s AbortController)"]
      EdgeEngine["MiniCPM5EdgeEngine<br/>(ONNX On-Device NLP)"]
      OnDeviceIntent["onDeviceIntentService<br/>(Rule-Based Fallback)"]
      i18n["i18n Module<br/>(EN / HI / GU)"]
      BiometricAuth["expo-local-authentication<br/>(Biometric + PIN)"]
    end
    
    AppEntry --> HomeScreen
    HomeScreen --> ZustandStore
  ...
  subgraph BackendAPI["FastAPI Backend :8000 (apps/backend/)"]
    direction TB
    MainPy["main.py<br/>(CORS + Router Mount)"]
    ...
    subgraph Services["Service Layer"]
      StateService["StateService<br/>(Core Orchestrator)"]
      SafetyPolicy["SafetyPolicyFilter<br/>(Anti-Predatory Lending)"]
      BehaviorEngine["BehavioralEngine<br/>(Contextual Habits)"]
      ExpComposer["ExperienceComposer<br/>(State -> UI Config)"]
    end
    
    subgraph DataAccess["Data Access Layer"]
      DataLoader["DataLoader<br/>(DB -> JSON Fallback)"]
    ...
    MainPy --> CustomerRoute
    CustomerRoute --> StateService
    StateService --> DataLoader
  ...
  subgraph AIEngine["AI Intelligence Pipeline (ai/)"]
    ...
    subgraph Ingestion["Data Ingestion"]
      Harmonizer["harmonizer.py<br/>(MD5 Dedup + ISO Parse)"]
    end
    
    subgraph FeatureExt["Feature Extraction"]
      Extractor["extractor.py<br/>(50/30/20 Rule + 32 Metrics)"]
      SpendAnalyzer["spend_analyzer.py<br/>(9-Category Breakdown)"]
      Forecaster["forecaster.py<br/>(30-Day Runway)"]
    end
    
    subgraph SignalDet["Signal Detection"]
      BehavioralSig["behavioral.py<br/>(Commute, Dining, Cinema)"]
      FinancialSig["financial.py<br/>(DTI, Risk, Savings Decline)"]
      LifecycleSig["lifecycle.py<br/>(Medical, Marriage, Job Change)"]
    end
    
    subgraph Personalization["Personalization Engine"]
      StateGenerator["generator.py<br/>(Dual Cadence Controller)"]
      Scorer["scorer.py<br/>(Multi-Factor Utility)"]
      ComplianceFilter["compliance.py<br/>(RBI DTI + DPDP)"]
      CryptoLedger["cryptoledger.py<br/>(SHA-256 Chain)"]
    end
    ...
    IngestionPipeline --> RawFeatures
    RawFeatures --> DetectedSignals
    DetectedSignals --> EnginePersonalize
    EnginePersonalize --> CheckpointPropensity
  end

  subgraph VoiceAI["On-Device Voice SLM (ai/voice/)"]
    MiniCPM["MiniCPM-5 INT4<br/>(~1.85GB RAM)"]
    IntentClassifier["Intent Classifier<br/>(PAY_METRO, CHECK_EMI, etc.)"]
  ...
  subgraph DataLayer["Data Layer"]
    PostgreSQL[("PostgreSQL 16<br/>(Docker :5432)<br/>9 Tables")]
    SeedJSON["JSON Seed Data<br/>(1.2k Customers<br/>127k Transactions)"]
    ScenarioJSON["Scenario Overrides<br/>(normal, stress, medical,<br/>fraud, life-change)"]
  end
  ...
  U1 -->|Touch or Voice| HomeScreen
  HomeScreen -->|REST API via JSON| MainPy
  DialogueManager -->|voice-intent.schema.json| MainPy
  ExpComposer -->|experience.schema.json| HomeScreen
  EnginePersonalize -->|customer-state.schema.json| StateService
  DataLoader --> PostgreSQL
  PostgreSQL -.->|DB Offline Fallback| SeedJSON
  CBS --> IngestionPipeline
  StateService -->|Triggers Pipeline| EnginePersonalize
...
This is the core architectural pattern of ABC Bank. The AI determines the customer's reality, the Backend synthesizes it into a UI specification, and the Frontend blindly renders whatever the Backend tells it to show. This means the AI team can completely change the user experience without modifying any React Native code.

```mermaid
flowchart LR
  subgraph AILayer["AI Layer (Ubaid)"]
    direction TB
    DataSources["7 Data Sources<br/>(CBS, UPI, SMS, CIBIL,<br/>BBPS, NCMC, KYC)"]
    Pipeline["Intelligence Pipeline<br/>(Harmonize -> Extract -> <br/>Detect -> Score -> Comply)"]
    CryptoLog["SHA-256 Crypto Ledger<br/>(Decision Audit Trail)"]
    CustomerState["CustomerState JSON<br/>- financial_health: stress<br/>- signals: [commute, emi_due]<br/>- recommendations: [<br/> {product: 'metro_pass',<br/> suppressed: false},<br/> {product: 'personal_loan',<br/> suppressed: true}<br/>]"]
 ...
 Pipeline --> CryptoLog
  ...
  subgraph BackendLayer["Backend Layer (Harsh)"]
    direction TB
    StateService2["StateService<br/>(Orchestrator)"]
    Composer["ExperienceComposer<br/>(Attention Layer Mapping)"]
    Safety["SafetyPolicyFilter<br/>(DTI above 0.40 -> Suppress)"]
    BehavEngine2["BehavioralEngine<br/>(Time-Aware Context)"]
    ExperienceConfig["ExperienceConfig JSON<br/>- hero_card: 'EMI Due Tomorrow'<br/>- context_cards: [<br/> {layer: DO, title: 'Pay EMI'},<br/> {layer: KNOW, title: 'Spending Up 12%'},<br/> {layer: PLAN, title: 'SIP Investment'}<br/>]<br/>- primary_actions: ['Pay', 'Transfer']<br/>- deprioritized: ['Personal Loan']"]
    
    StateService2 --> Composer
 Composer --> Safety
  ...
  subgraph FrontendLayer["Frontend Layer (Lakshya)"]
    direction TB
    AppNavigator["AppNavigator<br/>(Custom Zustand Router)"]
    AdaptiveHome["AdaptiveHomeScreen<br/>(Renders SDUI Config)"]
    HeroCard["Hero Card Component<br/>(Dynamic based on state)"]
    ContextCards["Context Card Stack<br/>(DO -> KNOW -> PLAN -> CONSIDER)"]
    ActionBar["Primary Action Bar<br/>(Dynamic buttons)"]
  ...
  CustomerState -->|contracts or customer-state.schema.json| StateService2
  ExperienceConfig -->|contracts or experience.schema.json| AppNavigator
...
flowchart TD
  subgraph Sources["7 Raw Data Sources"]
    CBS2["Core Banking<br/>System (CBS)"]
    UPI2["UPI Transaction<br/>Logs"]
    SMS2["Android SMS<br/>Parser"]
    CIBIL2["CIBIL Credit<br/>Bureau"]
    BBPS2["BBPS Bill<br/>Payments"]
    NCMC2["NCMC Metro<br/>Card Data"]
    KYC2["KYC/Demographics"]
  end

  subgraph Ingestion2["Data Ingestion (ai/intelligence/ingestion/)"]
    MD5["MD5 Deduplication<br/>(Idempotent Transaction IDs)"]
    ISOParse["ISO 8601 Date<br/>Normalization"]
    ConflictRes["Balance Conflict Resolution<br/>(balance_conflicts_resolved)"]
    InMemCache["O(1) In-Memory<br/>Feature Cache"]
  end

  subgraph Features2["Feature Extraction (ai/intelligence/features/)"]
    FiftyThirtyTwenty["50/30/20 Rule Analysis<br/>(Needs/Wants/Savings Split)"]
    NineCategories["9-Category Spend<br/>Breakdown"]
    ThirtyDayForecast["30-Day Cash Flow<br/>Runway Forecast"]
    ThirtyTwoMetrics["32 Single-Pass<br/>Feature Metrics"]
  end

  subgraph Signals2["Signal Detection (ai/intelligence/signals/)"]
    BehavSignals["Behavioral Signals<br/>(Commute Pattern, Dining,<br/>Cinema, Weekend Habits)"]
    FinSignals["Financial Signals<br/>(DTI Ratio, EMI Pressure,<br/>Savings Decline, Risk Score)"]
    LifeSignals["Lifecycle Signals<br/>(Medical Emergency, Marriage,<br/>Job Change, Retirement)"]
  end

  subgraph StateGen2["Customer State (ai/intelligence/customer_state/)"]
    DualCadence["Dual Cadence Controller<br/>(14-day Macro + 0.162ms Micro)"]
    StateBuilder["Customer State Builder<br/>(financial_health Classification)"]
    HealthClass["Financial Health:<br/>thriving / stable / tight / stress"]
  end

  subgraph PersonalizationEng["Personalization (ai/intelligence/personalization/)"]
    MultiFactorScorer["Multi-Factor Utility Scorer<br/>30% Affordability<br/>30% Lifecycle Match<br/>25% Urgency<br/>15% Archetype Fit<br/>- Risk Penalty"]
    ArchetypeMatch["7 Bharat Archetypes<br/>(urban_commuter, rural_farmer,<br/>salaried_professional, student,<br/>small_business_owner,<br/>retired_senior, gig_worker)"]
    RBICompliance["RBI Compliance Check<br/>(DTI above 0.40 -> Suppress All Credit)"]
    DPDPCheck["DPDP Purpose Limitation<br/>(Data Protection Compliance)"]
    CryptoLedger2["SHA-256 Crypto Ledger<br/>(Tamper-Proof Decision Chain)"]
  end

  subgraph ExperienceComp["Experience Composition (apps/backend/)"]
    AttentionLayers["Attention Layer Classification<br/>- DO: Immediate (EMI Due)<br/>- KNOW: Awareness (Spend up12%)<br/>- PLAN: Future (SIP, Insurance)<br/>- CONSIDER: Low Priority"]
    HeroCardGen["Hero Card Generation<br/>(Most Urgent Item)"]
    SafetyPolicyFilter["Safety Policy Filter<br/>(Block Predatory Lending<br/>for Stressed Customers)"]
    ModuleComposition["Dynamic Module<br/>Composition"]
  end

  subgraph UIRender["Adaptive UI Render"]
    AdaptiveUI["AdaptiveHomeScreen<br/>(Morphs Per State)"]
    NormalUI["Normal State:<br/>Standard Dashboard"]
    StressUI["Stress State:<br/>Empathetic Hero Card<br/>+ Hidden Utility Widgets"]
    MedicalUI["Medical State:<br/>Medical Assistance<br/>Prominent"]
  end

  CBS2 --> MD5
  MD5 --> ISOParse
  ISOParse --> ConflictRes
  ConflictRes --> FiftyThirtyTwenty
  FiftyThirtyTwenty --> ThirtyTwoMetrics
  NineCategories --> ThirtyTwoMetrics
  ThirtyDayForecast --> ThirtyTwoMetrics
  ThirtyTwoMetrics --> CommuteSig2
  CommuteSig2 --> DualCadence
  DualCadence --> StateBuilder
  StateBuilder --> HealthClass
  HealthClass --> MultiFactorScorer
  ArchetypeMatch --> MultiFactorScorer
  MultiFactorScorer --> RBICompliance
  RBICompliance --> DPDPCheck
  DPDPCheck --> CryptoLedger2
  CryptoLedger2 -->|customer-state.json| AttentionLayers
  AttentionLayers --> HeroCardGen
  HeroCardGen --> SafetyPolicyFilter
  SafetyPolicyFilter --> ModuleComposition
  ModuleComposition -->|experience-config.json| AdaptiveUI
  HealthClass -->|stress| StressUI
  HealthClass -->|thriving or stable| NormalUI
...
flowchart TD
  ...
  subgraph DockerEnv["Docker Environment"]
    PostgresContainer["postgres:16-alpine<br/> Port 5432<br/> Volume: postgres_data<br/> Healthcheck: pg_isready"]
  end

  subgraph PythonRuntime["Python Runtime"]
    FastAPIServer["FastAPI + Uvicorn<br/>Port 8000<br/>- 15+ REST Endpoints<br/>- CORS Middleware<br/>- Inspector Dashboards"]
    AIPipeline["AI Intelligence Pipeline<br/>- Feature Extraction<br/>- Signal Detection<br/>- Personalization<br/>- Crypto Ledger"]
  end

  subgraph MobileRuntime["Mobile Runtime"]
    ExpoDevServer["Expo Dev Server<br/>(Metro Bundler)"]
    ExpoGoApp["Expo Go App<br/>(Physical Device)"]
    ONNXRuntime["ONNX Runtime<br/>(On-Device ML)"]
    MiniCPMRuntime["MiniCPM-5 INT4 SLM<br/>(~1.85GB RAM on NPU)"]
  end

  subgraph ProductionHint["Production (Referenced)"]
    AWSRDS["AWS RDS PostgreSQL<br/>(Tunnel via localhost:5433)"]
  end

  VSCode --> PostgresContainer
  VSCode --> FastAPIServer
  VSCode --> ExpoDevServer
...
```mermaid
flowchart TB
  subgraph AIBoundary["AI Domain (Ubaid)<br/>ai/"]
    AIIngest["Data Ingestion<br/>(7 Sources)"]
    AIFeatures["Feature Extraction<br/>(32 Metrics)"]
    AISignals["Signal Detection"]
    AIPersonalize["Personalization Engine"]
    AICrypto["Crypto Ledger"]
    AIVoice["Voice SLM<br/>(MiniCPM-5)"]
  end

  subgraph Contract1["customer-state.schema.json"]
    CS["CustomerState<br/>- financial_health<br/>- signals[]<br/>- recommendations[]<br/> (each has suppressed flag)"]
  end

  subgraph Contract2["experience.schema.json"]
    EC["ExperienceConfig<br/>- hero_card<br/>- context_cards[]<br/> (DO/KNOW/PLAN/CONSIDER)<br/>- primary_actions[]<br/>- deprioritized_modules[]"]
  end

  subgraph Contract3["voice-intent.schema.json"]
    VI["VoiceIntent<br/>- intent (PAY_METRO, etc.)<br/>- language (en/hi/gu)<br/>- entities[]<br/>- response_text<br/>- confidence"]
  end

  subgraph BackendBoundary["Backend Domain (Harsh)<br/>apps/backend/"]
  ...
  subgraph FrontendBoundary["Frontend Domain (Lakshya)<br/>apps/frontend/"]
    FENav["AppNavigator"]
...
```mermaid
flowchart LR
  subgraph BackendAI["Backend Deterministic AI<br/>(Decision-Making Authority)"]
    direction TB
    MathScorer["Multi-Factor Utility Scorer<br/>(Pure Mathematics)"]
    DTICalc["DTI Ratio Calculator<br/>(Debt-to-Income)"]
  ...
  subgraph HardBoundary["HARD BOUNDARY<br/>(No Financial Data Crosses)"]
    Wall["Zero Banking Data<br/>Leakage Policy"]
  end

  subgraph DeviceSLM["On-Device SLM<br/>(Language-Only Authority)"]
    direction TB
    MiniCPM2["MiniCPM-5 INT4<br/>(~1.85GB RAM on NPU)"]
    VernacularNLP["Vernacular NLP<br/>(Hindi/Gujarati/Hinglish/EN)"]
    IntentExtract["Intent Extraction<br/>(PAY_METRO, CHECK_EMI)"]
    ResponseVerb["Response Verbalization<br/>(Pre-Computed Facts Only)"]
  ...
  CashFlow -->|Decisions and Suppressions| Wall
  Wall -->|Approved Trusted Facts Only| MiniCPM2
...
flowchart TD
  subgraph FrontendFeatures["Frontend Features"]
    F1["Adaptive Home Screen<br/>(Dynamic Dashboard)"]
    F2["Mitra AI Assistant<br/>(Voice + Chat)"]
    F3["1-Tap Payments<br/>(Biometric Auth)"]
    F4["Card Controls<br/>(Lock/Unlock/Limits)"]
    F5["Multilingual UI<br/>(EN/HI/GU)"]
    F6["Demo Scenario Switcher"]
    F7["SDUI Renderer<br/>(Dumb UI Layer)"]
    F8["Offline State Bundles"]
  end

  subgraph AIFeatures["AI Features"]
    A1["Ethical AI Personalization<br/>(Multi-Factor Scoring)"]
    A2["7 Bharat Archetypes"]
    A3["Financial Health Assessment<br/>(DTI/EMI/Savings)"]
    A4["Multi-Source Data Ingestion<br/>(7 Sources, MD5 Dedup)"]
    A5["Safe-to-Spend Analysis<br/>(50/30/20 + 30-Day Forecast)"]
    A6["Behavioral Context Engine<br/>(Time-Aware Suggestions)"]
    A7["Cryptographic Decision Ledger<br/>(SHA-256 Chain)"]
  end

  subgraph BackendFeatures["Backend Features"]
    B1["Experience Composer<br/>(SDUI Generation)"]
    B2["Safety Policy Filter<br/>(Anti-Predatory Lending)"]
    B3["PIN Authentication<br/>(PBKDF2-HMAC-SHA256)"]
    B4["Loan Disbursement<br/>(Max Rs. 1,50,000)"]
    B5["Inspector Dashboards<br/>(AI Debug Tools)"]
  end
...
flowchart TD
  ...
  subgraph SharedData["Shared Data"]
    SeedData["data/seed/<br/>(customers.json<br/>transactions.json)"]
    Scenarios["data/scenarios/<br/>(normal, stress, medical,<br/>fraud, life-change)"]
  end
  ...
  %% Dependencies
  AIIntelligence -->|Python import| BEServices
  AIVoice2 -->|Python import| BERoutes
  CSSchema -->|Validates| BEServices
  EXSchema -->|Validates| FEStore
  VISchema -->|Validates| BERoutes
  ...
  Audit -->|Validates| AIIntelligence
...
flowchart LR
  subgraph Producer1["AI Engine"]
    AIOut["Customer State<br/>Generator"]
  end

  subgraph Contract_CS["customer-state.schema.json<br/>(JSON Schema Draft-07)"]
    CSFields["- financial_health: thriving/stable/tight/stress<br/>- signals: [{type, confidence, source}]<br/>- recommendations: [{product_id, score,<br/> category, suppressed: bool,<br/> explanation}]"]
  end

  subgraph Producer2["Voice SLM"]
    VoiceOut["MiniCPM-5 Intent<br/>Classifier"]
  end

  subgraph Contract_VI["voice-intent.schema.json<br/>(JSON Schema Draft-07)"]
    VIFields["- intent: PAY_METRO/CHECK_EMI/...<br/>- language: en/hi/gu<br/>- entities: [{type, value}]<br/>- response_text: string<br/>- confidence: 0.0-1.0"]
  end

  subgraph Consumer1["Backend"]
    BEConsume["StateService<br/>+ ExperienceComposer"]
  end

  subgraph Contract_EX["experience.schema.json<br/>(JSON Schema Draft-07)"]
    EXFields["- hero_card: {title, subtitle, type, actions}<br/>- context_cards: [{layer: DO/KNOW/PLAN/CONSIDER,<br/> title, body, icon, actions}]<br/>- primary_actions: [{label, route}]<br/>- deprioritized_modules: [string]"]
  end

  subgraph Consumer2["Frontend"]
    FEConsume["Zustand Store -> <br/>AdaptiveHomeScreen"]
  end

  AIOut -->|Produces| CSFields
  CSFields -->|Consumed by| BEConsume
  VoiceOut -->|Produces| VIFields
  VIFields -->|Consumed by| BEConsume
  BEConsume -->|Produces| EXFields
  EXFields -->|Consumed by| FEConsume
...
flowchart TD
  subgraph EntryPoint["Entry Point"]
    MainPy2["main.py<br/>- FastAPI() init<br/>- CORSMiddleware (allow_origins=[*])<br/>- Router mount (/api/v1)<br/>- Static file serving<br/> (/inspector, /voice-inspector)"]
  end

  subgraph APILayer["API Layer (routes.py)"]
    CoreRoutes["Core Routes<br/>- GET /customer/{id}<br/>- GET /experience/{id}"]
    AuthRoutes["Auth Routes<br/>- POST /auth/pin/setup<br/>- POST /auth/pin/verify"]
    PayRoutes["Payment Routes<br/>- POST /payments/transfer<br/>- POST /loans/disburse"]
    CardRoutes["Card Routes<br/>- GET /cards/{id}<br/>- POST /cards/controls"]
    AssistantRoutes["Assistant Routes<br/>- GET /assistant/init<br/>- POST /assistant/chat<br/>- POST /assistant/intent"]
    EventRoutes["Event Routes<br/>- POST /events<br/>- POST /scenario/switch"]
  end

  subgraph ServiceLayer["Service Layer"]
    StateService2["StateService<br/>(Core Orchestrator)<br/>- get_state() -> cache/DB/AI/fallback<br/>- switch_scenario() -> flush + reload<br/>- ingest_event() -> balance + signals<br/>- execute_payment() -> lock + funds + ledger<br/>- disburse_loan() -> max Rs. 1,50,000<br/>- set/verify_customer_pin()<br/>- submit_kyc(), submit_medical_claim()<br/>- pause_mandate(), request_emi_grace()<br/>- split_emi(), sweep_deficit_for_emi()"]
    SafetyPolicy2["SafetyPolicyFilter<br/>- evaluate_financial_stress()<br/> (DTI above 0.40 or EMI pressure)<br/>- filter_recommendations()<br/> (suppress loan/payday/credit)<br/>- apply_module_policies()<br/> (restrict loan module,<br/> prioritize cashflow advisory)"]
    BehaviorEngine2["BehavioralEngine<br/>- analyze_habits()<br/> (recurring txn -> daily/weekly)<br/>- evaluate_current_relevance()<br/> (datetime.now() -> contextual hints<br/> Mon-Fri 07:45-09:30 = commute)"]
  end

  subgraph ExperienceLayer["Experience Layer"]
    ExpComposer2["ExperienceComposer<br/>- compose(state, recommendations)<br/> -> ExperienceConfigModel<br/>- _determine_layer(priority)<br/> -> DO/KNOW/PLAN/CONSIDER<br/>- Build hero_card<br/>- Build context_cards[]<br/>- Compute primary_actions[]<br/>- Mark deprioritized_modules[]"]
  end

  subgraph DataLayer2["Data Access Layer"]
    DataLoader2["DataLoader<br/>- Try PostgreSQL via repos<br/>- Catch SQLAlchemy exceptions<br/>- Fallback to JSON files<br/> (data/seed/*.json,<br/> data/scenarios/*.json)"]
    Repos["Repositories<br/>- CustomerRepository<br/> (get_by_id, get_summary)<br/>- TransactionRepository<br/> (get_by_customer, get_by_account)<br/>- EventRepository<br/>- ProductRepository"]
    Session["session.py<br/>- SQLAlchemy Engine<br/>- Connection pooling<br/>- Auto WSL IPv4 detection<br/> (PowerShell Get-NetNeighbor)<br/>- Fallback to localhost"]
  end

  subgraph PydanticModels["Pydantic Schemas"]
    CustState["CustomerStateModel<br/>(customer_id, financial_health,<br/>balances, signals, recommendations)"]
    ExpConfig["ExperienceConfigModel<br/>(hero_card, context_cards,<br/>primary_actions, deprioritized)"]
  ...
  MainPy2 --> CoreRoutes
  CoreRoutes --> StateService2
  StateService2 --> ExpComposer2
  ExpComposer2 --> SafetyPolicy2
  StateService2 --> DataLoader2
  DataLoader2 --> Repos
  Repos --> Session
  Session --> PostgreSQL2[("PostgreSQL 16")]
  DataLoader2 -.->|Fallback| JSONFiles["JSON Seed Files"]
  StateService2 -->|Python import| AIGenerator
```

### Request Lifecycle — Complete Sequence Diagram

This shows the exact execution path when a client requests `GET /experience/{customer_id}` — the core endpoint driving the adaptive UI:
...
sequenceDiagram
  ...
  participant Data as DataLoader / DB
  participant AI as AI Pipeline
  participant SDUI as ExperienceComposer

  Client->>API: GET /api/v1/experience/{id}
  API->>State: get_state(customer_id)
  
  alt Memory Cache Hit
    State-->>API: Return Cached CustomerState
  else Memory Cache Miss
    State->>Data: load_customer_data()
    Data-->>State: Customer Data (PostgreSQL or JSON Seed Fallback)
    State->>AI: generate_state(features, signals)
    AI-->>State: Enriched CustomerState
    State->>State: Cache in Memory (O(1) TTL)
  end
  
  State->>SDUI: compose(state, recommendations)
  SDUI->>SDUI: Apply Safety Policy Filter (DTI Cap 0.40)
  SDUI->>SDUI: Classify Attention Layers (DO, KNOW, PLAN, CONSIDER)
  SDUI-->>State: Final ExperienceConfigModel
  State-->>API: ExperienceConfig JSON
...
```mermaid
flowchart TD
  Start["get_state: customer_id"] --> CacheCheck{"Memory Cache Hit?"}
  CacheCheck -->|Yes| ReturnCached["Return Cached State: O(1) Lookup"]
  CacheCheck -->|No| LoadData["DataLoader: load_customer_data"]
  
  LoadData --> DBCheck{"PostgreSQL Reachable?"}
  DBCheck -->|Yes| QueryDB["Query via Repositories"]
  DBCheck -->|No| JSONFallback["Fallback: Read JSON Seed Files"]
  
  QueryDB --> RunAI["Run AI Intelligence Pipeline"]
  JSONFallback --> RunAI
  
  RunAI --> AICheck{"Pipeline Success?"}
  AICheck -->|Yes| BuildState["Build CustomerStateModel and Apply Behavioral Engine"]
  AICheck -->|No| SafeFallback["Create Safe Fallback Profile"]
...
```mermaid
flowchart TD
  Input["CustomerStateModel and AI Recommendations"] --> LayerClassify["Classify into Attention Layers"]
  
  LayerClassify --> DO["DO Layer: Priority 8+ (Overdue EMI, Card Lock, Fraud)"]
  LayerClassify --> KNOW["KNOW Layer: Priority 5-7 (Spend Spike, Balance Alert)"]
  LayerClassify --> PLAN["PLAN Layer: Priority 3-4 (SIP Investment, Insurance)"]
  LayerClassify --> CONSIDER["CONSIDER Layer: Priority 1-2 (Card Upgrade, Minor Offers)"]
  
  DO --> HeroCard["Generate Hero Card: Most Urgent Item"]
  KNOW --> ContextCards["Build Context Cards Array"]
  PLAN --> ContextCards
  ...
  DraftConfig --> SafetyCheck{"SafetyPolicyFilter: Stressed?"}
  SafetyCheck -->|DTI above 0.40| Suppress["Suppress Credit Products and Hide Loans"]
  SafetyCheck -->|Healthy| PassThrough["Pass Through: All Recommendations Visible"]
  
  Suppress --> FinalConfig["Final ExperienceConfigModel"]
  PassThrough --> FinalConfig
  
  FinalConfig --> OutputNode["JSON Response to Frontend"]
```
...
```mermaid
flowchart TD
  Input2["Draft Recommendations from AI Engine"] --> StressEval{"Evaluate Financial Stress"}
  
  StressEval -->|DTI above 0.40| STRESSED["STRESSED"]
  ...
  STRESSED --> FilterLoop["Filter Recommendations Loop"]
  
  FilterLoop --> CheckCategory{"Category Match?"}
  CheckCategory -->|Borrowing Products| SuppressRec["SET suppressed = true - SET priority = 0 - Suppressed due to financial stress"]
  CheckCategory -->|Safety and Savings Products| KeepRec["KEEP recommendation - May increase priority"]
  
  SuppressRec --> ModulePolicies["Apply Module Policies"]
  KeepRec --> ModulePolicies
  
  HEALTHY --> ModulePolicies
  
  ModulePolicies --> RestrictLoan["Restrict Loan Module Display"]
  ModulePolicies --> PrioritizeCashflow["Prioritize Cashflow Advisory Modules"]
  
  RestrictLoan --> SafeConfig["Safe Configuration Output"]
...
sequenceDiagram
  ...
  User->>Store: Tap Pay Metro Rs. 40
...
```mermaid
flowchart TD
  AppTsx["App.tsx Entry"] --> ThemeProvider["ThemeProvider"]
  ThemeProvider --> Navigator["AppNavigator Router"]
  
  Navigator --> HomeScreen["AdaptiveHomeScreen (SDUI Engine)"]
  Navigator --> ChatScreen["MitraChatScreen (AI Assistant)"]
  Navigator --> PayScreen["PaymentsScreen (1-Tap and Mandates)"]
  Navigator --> TxnScreen["TransactionsScreen (Activity Logs)"]
  Navigator --> ModalOverlay["Journey Modals (KYC, Insurance, Medical)"]
  
  HomeScreen --> Store["useCustomerStore (Zustand Core)"]
  ChatScreen --> Store
  PayScreen --> Store
  ModalOverlay --> Store
  
  Store --> Api["BankingApi Client"]
  Api -.->|Offline Fallback| EdgeEngine["MiniCPM5EdgeEngine (On-Device NLP)"]
  
  HomeScreen --> ContextCards["Adaptive Context Cards (DO, KNOW, PLAN)"]
  ContextCards --> ThemeTokens["Design Tokens (Colors and Typography)"]
```
...
```mermaid
flowchart TD
  Launch["App Launch"] --> Home["AdaptiveHomeScreen (Main Dashboard)"]
  
  Home <--> TabPay["PaymentsScreen Tab"]
  Home <--> TabTxn["TransactionsScreen Tab"]
  Home <--> TabInsights["InsightsScreen Tab"]
  Home <--> TabProfile["ProfileScreen Tab"]
  Home <--> TabMore["More / Settings Tab"]
  
  Home --> KYC["KYC Journey Modal"]
  Home --> Medical["Medical Emergency Modal"]
  Home --> Insurance["Insurance Claim Modal"]
  TabMore --> DemoLab["Demo Lab Scenario Switcher"]
  
  KYC -.->|Dismiss| Home
  Medical -.->|Dismiss| Home
  Insurance -.->|Dismiss| Home
  DemoLab -.->|Dismiss| TabMore
```
...
```mermaid
flowchart TD
  AppInit["App / Pull-to-Refresh: fetchStateAndContext()"] --> APIReq["BankingApi: GET /api/v1/experience/{id}"]
  
  APIReq --> TimeoutGuard{"Request Status (2.5s Timeout Guard)"}
  
  TimeoutGuard -->|Online 200 OK| LiveSuccess["Live Backend: Return ExperienceConfig JSON"]
  TimeoutGuard -->|Timeout > 2.5s| EdgeEngine["Edge Fallback: MiniCPM-5 On-Device NLP and ONNX"]
  TimeoutGuard -->|Network Error| CacheBundle["Cache Fallback: Load Offline Scenario Bundle"]
  
  LiveSuccess --> UpdateStore["useCustomerStore: Update Reactive State"]
  EdgeEngine --> UpdateStore
  CacheBundle --> UpdateStore
  
  UpdateStore --> RenderHome["AdaptiveHomeScreen: Re-render Dynamic Dashboard"]
```
...
```mermaid
flowchart TD
  UserTap["User: Tap Payment Mandate (e.g. Metro Rs. 40)"] --> TriggerAuth["Store: requestPaymentAuth()"]
  TriggerAuth --> DeviceAuth{"Device Biometrics Available?"}
  
  DeviceAuth -->|Yes| BioScan["expo-local-authentication: Biometric Scan"]
  DeviceAuth -->|No| PINInput["MPIN Fallback: POST /auth/pin/verify"]
  
  BioScan --> BioResult{"Scan Passed?"}
  BioResult -->|Yes| ExecPay["POST /payments/transfer"]
  BioResult -->|No| ErrToast["Show Error Toast: Authentication Failed"]
  
  PINInput --> PINResult{"PIN Valid?"}
  PINResult -->|Yes| ExecPay
  PINResult -->|No| ErrToast
  
  ExecPay --> SuccessResp["200 OK: Balance Updated and Mandate Paid"]
  SuccessResp --> UIFeedback["AdaptiveHomeScreen: Animated Checkmark"]
```
...
```mermaid
flowchart TD
  ExConfig["ExperienceConfig from Backend"] --> ScenarioDetect{"Active Scenario State"}
  
  ScenarioDetect -->|Normal or Surplus| NormalHero["Hero: Welcome and Balance"]
  NormalHero --> NormalCards["Context Cards: Insights and Tips"]
  NormalCards --> NormalActions["Actions: Pay, Transfer, Scan"]
  
  ScenarioDetect -->|Financial Stress| StressHero["Hero: Wellness Support"]
  StressHero --> StressCards["Context Cards: Cashflow Advisory"]
  StressCards --> StressActions["Actions: Budgeting and Relief Options"]
  
  ScenarioDetect -->|Medical Event| MedHero["Hero: Emergency Assistance"]
  MedHero --> MedCards["Context Cards: Insurance Claims and Instant Loan"]
  
  ScenarioDetect -->|Fraud Alert| FraudHero["Hero: Security Alert"]
  FraudHero --> FraudCards["Context Cards: Review Suspicious Activity"]
```
...
```mermaid
flowchart TD
  UserAction["User Interaction: Tap, Pull, Navigate"] --> StoreDispatch["Store Action Dispatcher"]
  
  StoreDispatch --> Router{"Operation Type"}
  Router -->|Network Fetch| ApiCall["BankingApi HTTP Client"]
  Router -.->|Offline Fallback| EdgeEngine["MiniCPM5EdgeEngine Local NLP"]
  Router -->|Authentication| BioService["expo-local-authentication"]
  
  ApiCall --> StoreCore["useCustomerStore: Zustand Central Store"]
  EdgeEngine --> StoreCore
  BioService --> StoreCore
  
  StoreCore --> ReactiveUI["Reactive UI: AdaptiveHomeScreen, MitraChatScreen, PaymentsScreen"]
```
...
```mermaid
flowchart TD
  DataSources["7 Raw Data Sources (CBS, UPI, SMS, CIBIL, BBPS, NCMC, KYC)"] --> Harmonization["Data Harmonization (harmonizer.py: MD5 Dedup, ISO Dates, Conflict Resolution)"]
  Harmonization --> FeatureExtraction["Feature Extraction (50/30/20 Rule, 9 Spend Categories, Cash Flow Forecast)"]
  FeatureExtraction --> SignalDetection["Signal Detection (Commute Habits, EMI Pressure, Life Events, DTI Ratio)"]
  SignalDetection --> StateGeneration["Customer State Generation (Financial Health Classifier: Thriving, Stable, Tight, Stress)"]
  StateGeneration --> RecommendationEngine["Recommendation Engine (Multi-Factor Utility Scorer + 7 Bharat Archetypes)"]
  RecommendationEngine --> ComplianceLayer["Compliance Layer (RBI DTI 0.40 Cap, DPDP Purpose, SHA-256 Crypto Ledger)"]
  ComplianceLayer --> OutputState["customer-state.json to Backend ExperienceComposer"]
```
...
```mermaid
flowchart TD
  UserSpeech["User Spoken Utterance: Mera khata balance kitna hai?"] --> STT["Speech-to-Text Conversion (Web Speech API / Native)"]
  STT --> SLM["On-Device MiniCPM-5 INT4 SLM (~1.85GB RAM)"]
  
  SLM --> PolicyCheck["Zero-Leakage Policy: No Raw Banking History in Prompt"]
  PolicyCheck --> Intent["Classify Intent: CHECK_BALANCE (Confidence 0.94)"]
  
  Intent --> BackendFetch["POST /assistant/intent to FastAPI Backend"]
  BackendFetch --> FactStore["Database / Trusted Fact Store: Fetch Balance (Rs. 45,230)"]
  
  FactStore --> ResponseVerbalizer["On-Device SLM: Verbalize Fact in Hindi"]
  ResponseVerbalizer --> AudioOut["Audio Output: Aapka khata balance Rs. 45,230 hai"]
```
...
flowchart LR
  subgraph Visible["Yes Data SLM CAN Access"]
    Intent["User's spoken intent<br/>(natural language text)"]
    TrustedFacts2["Pre-computed trusted facts<br/>- Current balance<br/>- Next EMI due date<br/>- Card lock status<br/>- Bill amounts"]
    Templates["Response templates<br/>(Localized strings)"]
  end
  ...
  Intent -->|Allowed Spoken Input| Parse
  TrustedFacts2 -->|Allowed Pre-computed Facts| Verbalize
  TxnHistory -.->|BLOCKED by Zero-Leakage Policy| Parse
  PersonalData -.->|BLOCKED by Zero-Leakage Policy| Parse
...
flowchart TD
  ...
  subgraph DataAccessDB["Data Access Layer"]
    DataLoaderDB["DataLoader<br/>(Primary Access Point)"]
    
    subgraph Repos["Repositories"]
      CustRepoDB["CustomerRepository<br/>- get_by_id()<br/>- get_customer_summary()"]
      TxnRepoDB["TransactionRepository<br/>- get_by_customer()<br/>- get_by_account()"]
      EvtRepoDB["EventRepository<br/>- get_events()"]
      ProdRepoDB["ProductRepository<br/>- get_products()"]
    end
  end

  subgraph ORMLayer["ORM Layer"]
    SQLAlchemy["SQLAlchemy 2.0<br/>(Declarative Models)"]
    SessionDB["session.py<br/>- Engine + Connection Pool<br/>- Auto WSL IPv4 detection<br/> (PowerShell Get-NetNeighbor)<br/>- Fallback: localhost"]
  end

  subgraph StorageDB["Storage"]
    PostgreSQLDB[("PostgreSQL 16<br/>(Docker :5432)<br/>9 Tables")]
    JSONSeedDB["JSON Seed Files<br/>(data/seed/)<br/>- customers.json (1.2k)<br/>- transactions.json (127k+)"]
    ScenarioFilesDB["Scenario Overrides<br/>(data/scenarios/)<br/>- normal.json<br/>- financial-stress.json<br/>- life-change.json"]
  end

  Route --> StateServiceDB
  StateServiceDB --> DataLoaderDB
  DataLoaderDB --> CustRepoDB
  CustRepoDB --> SQLAlchemy
  SQLAlchemy --> SessionDB
  SessionDB --> PostgreSQLDB
  
  DataLoaderDB -.->|PostgreSQL Offline Fallback| JSONSeedDB
...
flowchart LR
  subgraph Development["Development"]
    Dev["Developer writes<br/>migration"]
  end

  subgraph AlembicMgr["Alembic Migration Manager"]
    AlembicEnv["alembic/env.py"]
    AlembicVer["alembic/versions/<br/>(Migration Files)"]
    AlembicIni["alembic.ini<br/>(Configuration)"]
  end

  subgraph Database3["PostgreSQL"]
    Schema["Live Database Schema<br/>(9 Tables)"]
    AlembicTable["alembic_version table<br/>(Tracks current version)"]
  end

  subgraph PrismaTool["Prisma (Read-Only)"]
    PrismaIntrospect["prisma db pull<br/>(Introspects live DB)"]
    PrismaSchema["prisma/schema.prisma<br/>(Generated schema file)"]
    PrismaStudio["Prisma Studio<br/>(Visual DB Browser)"]
  end
...
```mermaid
flowchart TD
  Generator["scripts/generate_seed_data.py<br/>(~540 lines)"]
  Generator -->|Generates| CustJSON["data/seed/customers.json<br/>(1,200 Mock Profiles)"]
  Generator -->|Generates| TxnJSON["data/seed/transactions.json<br/>(127,000+ Transactions)"]
  
  Seeder["scripts/seed_database.py"]
  CustJSON --> Seeder
  TxnJSON --> Seeder
  
  Seeder -->|SQLAlchemy bulk_insert_mappings in 10k batches| DB2[("PostgreSQL 16")]
  
  Seeder -->|Step 1| Wipe["Wipe existing data<br/>(Clean slate)"]
  Seeder -->|Step 2| Reset["Reset schema<br/>(Recreate tables)"]
  Seeder -->|Step 3| Insert["Bulk insert<br/>(10k per batch)"]
  
  ScenarioFiles["data/scenarios/<br/>- normal.json<br/>- financial-stress.json<br/>- life-change.json"]
  ScenarioFiles -->|Runtime Override| StateService3["StateService<br/>(switch_scenario)"]
```
...
ABC Bank implements a banking-grade customer authentication flow combining mobile OTP verification, 4-digit MPIN credentials (or device biometrics), and statutory DPDP Act 2023 compliance agreements.
...
sequenceDiagram
  ...
  Note over User, UI: Stage 1: Phone Entry and KYC Identifier
  ...
  alt First-time Customer
    ...
    AuthAPI-->>UI: PIN Hash and Salt persistent registration
  ...
  Note over User, UI: Stage 4: DPDP Act 2023 and RBI Statutory Consent
  alt New Account / Updated Compliance Scope
    UI->>User: Display DPDP Statutory Disclosures Modal
    Note over UI: Disclose Data Fiduciary, Purpose, Account Aggregator AA,<br/>Right to Revoke and Grievance DPO Contact
    User->>UI: Accepts statutory terms and grants consent
  ...
  Store-->>UI: Hydrate active scenario and banking dashboard
  UI-->>User: Transition to Main Application Shell
```

#### Dual Authentication Architecture (Mobile OTP / MPIN & NetBanking JWT)

ABC Bank supports two complementary authentication paths designed to accommodate both modern Bharat mobile banking and standard internet banking / developer evaluation:
...
In compliance with the **Digital Personal Data Protection (DPDP) Act, 2023** and **RBI Master Direction on IT Governance, Risk and Controls**, ABC Bank enforces strict transparency, consent gating, and revocation capabilities before any financial or behavioral data is processed.

#### Statutory Principles Enforced

| Dimension | DPDP 2023 Requirement | ABC Bank Implementation |
|---|---|---|
| **Data Fiduciary Identity** | Clear disclosure of processing entity | Displayed as *ABC Bank Ltd.* |
...
| **Grievance Redressal** | Mandatory designation of Data Protection Officer (DPO) | Statutory DPO contact (`dpo@abcbank.in`, Grievance Officer, Mumbai) accessible directly from the consent sheet. |
...
flowchart TD
  ...
  UserAction -->|Reject or Exit| TerminateSession[Session Blocked / Guest Only]
  UserAction -->|Accept| RecordConsent[Store SHA-256 Consent Proof and Timestamp]
  RecordConsent --> UnlockVault[Unlock Full Banking SDUI and Accounts]
  ...
  RevokeAction -->|Yes| StripTelemetry[Anonymize Local Cache and Block Personalization]
```
...
stateDiagram-v2
  ...
  state PINActive {
    ...
    IncrementFails --> Ready: fail_count under 5
...
sequenceDiagram
  ...
  Client->>Router: POST /auth/pin/verify - {customer_id, pin}
  ...
  alt No PIN Set
    ...
    alt Currently Locked
      ...
      Router-->>Client: {error: Account locked, - retry_after: 15 minutes}
    else Not Locked
      ...
      alt Match
        ...
        alt failed_attempts >= 5
          ...
          Auth-->>Router: 429 Rate Limited
          Router-->>Client: {error: Too many attempts, - locked_for: 15 minutes}
        else failed_attempts under 5
          Auth-->>Router: 401 Unauthorized
          Router-->>Client: {error: Invalid PIN, - attempts_remaining: N}
        end
      end
    end
  end
...
stateDiagram-v2
  ...
  Unlocked --> Locked: POST /cards/controls (lock card)
  Locked --> Unlocked: POST /cards/controls (unlock card)

  Locked --> PaymentDenied: POST /payments/transfer (ATM/POS/Card)
...
```mermaid
flowchart TD
  Request["Incoming API Request"] --> Layer1{"Layer 1:<br/>Customer ID<br/>Exists?"}
  
  Layer1 -->|No| Reject404["404 Not Found"]
  Layer1 -->|Yes| Layer2{"Layer 2:<br/>PIN Locked Out?<br/>(Rate Limiting)"}
  
  Layer2 -->|Locked - 15min cooldown| Reject429["429 Rate Limited"]
  Layer2 -->|Not Locked| Layer3{"Layer 3:<br/>Card Locked?<br/>(For Payments)"}
  
  Layer3 -->|Locked| Reject403["403 Forbidden<br/>'Card is locked'"]
  Layer3 -->|Active| Layer4{"Layer 4:<br/>Sufficient<br/>Funds?"}
  
  Layer4 -->|Insufficient| Reject400["400 Bad Request<br/>'Insufficient balance'"]
  Layer4 -->|Sufficient| Layer5{"Layer 5:<br/>Safety Policy<br/>(For Recommendations)"}
  
  Layer5 -->|DTI above 0.40| Suppress["Suppress Credit Products<br/>(suppressed = true)"]
  Layer5 -->|Healthy| Allow["Allow All Products"]
  
  Suppress --> Layer6["Layer 6:<br/>Crypto Ledger<br/>(Log Decision)"]
  Allow --> Layer6
...
```mermaid
flowchart TD
  AIRec["AI Recommendation<br/>Generated"] --> DTICheck{"DTI Ratio<br/>Check"}
  
  DTICheck -->|DTI above 0.40| BLOCK["BLOCK"]
  DTICheck -->|DTI <= 0.40| StressCheck{"Financial Health<br/>Classification"}
  
  StressCheck -->|stress| BLOCK
  StressCheck -->|tight| CAUTION["CAUTION<br/>(Flag but allow)"]
  StressCheck -->|stable or thriving| ALLOW["ALLOW"]
  
  BLOCK --> SuppressAll["Suppress ALL:<br/>- Personal loans<br/>- Payday advances<br/>- Credit limit increases<br/>- Credit card offers"]
  BLOCK --> ReplaceWith["Replace with:<br/>- Cashflow advisory<br/>- Emergency fund tips<br/>- Budget optimization<br/>- Auto-sweep FD suggestions"]
  
  SuppressAll --> CryptoLog["SHA-256 Crypto Ledger<br/>Log suppression with:<br/>- customer_id<br/>- product_id<br/>- reason: 'DTI 0.45 exceeds 0.40'<br/>- timestamp<br/>- hash_chain_link"]
  ReplaceWith --> CryptoLog
  
  CAUTION --> CryptoLog
  ALLOW --> CryptoLog
  
  CryptoLog --> Explanation["Counterfactual Explanation<br/>'This loan was not shown because<br/>your debt-to-income ratio (0.45)<br/>exceeds the RBI guideline of 0.40'"]
  
  Explanation --> FinalOutput["Final Recommendation<br/>List (Filtered)"]
```
...
sequenceDiagram
  ...
  DS->>Harm: Raw banking data (CBS, UPI, SMS, - CIBIL, BBPS, NCMC, KYC)
  ...
  Comply->>Comply: RBI DTI cap check (above 0.40?)
...
```mermaid
flowchart TD
  subgraph DOLayer["DO Layer (Priority >= 8)<br/>Immediate Action Required"]
  ...
  subgraph KNOWLayer["KNOW Layer (Priority 5-7)<br/>Awareness Items"]
    KNOW1["Spending Up 12% This Month"]
    KNOW2["Balance Below Rs. 5,000"]
    KNOW3["Bill Due in 3 Days"]
    KNOW4["Salary Credit Expected"]
    KNOW5["FD Maturity This Week"]
  end

  subgraph PLANLayer["PLAN Layer (Priority 3-4)<br/>Future Planning"]
  ...
  subgraph CONSIDERLayer["CONSIDER Layer (Priority 1-2)<br/>Low Priority Suggestions"]
    CONSIDER1["Credit Card Upgrade Available"]
    CONSIDER2["Savings Account Comparison"]
    CONSIDER3["Loyalty Reward Points"]
  end

  DO1 -->|Highest Priority Hero Card Source| HeroCard2["Hero Card<br/>(Most Urgent DO Item)"]
  KNOW1 -->|Context Cards| ContextStack["Context Card Stack"]
  PLAN1 -->|Context Cards| ContextStack
  CONSIDER1 -->|Deprioritized Cards| ContextStack
...
```mermaid
flowchart TD
  subgraph AITests["AI Test Suite (ai/tests/)<br/>45 Tests, ~0.39s"]
    T1["test_massive_load.py<br/>10k transactions under 100ms"]
    T2["test_multi_source_ingestion.py<br/>SMS vs CBS conflict arbitration"]
    T3["test_personalization.py<br/>Ethical loan suppression<br/>Top-1-to-5 ranking"]
    T4["test_cadence.py<br/>Dual cadence timing"]
    T5["test_cryptoledger.py<br/>SHA-256 chain integrity"]
    T6["test_voice_slm.py<br/>Voice pipeline testing"]
  end

  subgraph AuditSuite["Audit Suite (scripts/)<br/>39 Checks, ~540 Lines"]
    A1["ONNX inference latency<br/>(under 10ms requirement)"]
    A2["Subword tokenizer<br/>vector integrity"]
    A3["XGBoost propensity<br/>extraction accuracy"]
    A4["KMeans archetype<br/>soft-clustering validity"]
    A5["DTI loan suppression<br/>(under 0.40 threshold)"]
    A6["50/30/20 spend<br/>breakdown accuracy"]
    A7["SHA-256 crypto ledger<br/>integrity verification"]
    A8["Multi-turn vernacular<br/>chatbot routing"]
  end
  ...
  T1 -->|pytest test suite| Results["111 Tests Passing<br/>(45 AI + 66 Backend)"]
  A1 -->|39-check audit run| Results
...
flowchart TD
  ...
  subgraph LocalInfra["Local Infrastructure"]
    DockerDB["Docker Compose<br/>- postgres:16-alpine<br/>- Port 5432<br/>- Volume: postgres_data"]
    FastAPILocal["FastAPI + Uvicorn<br/>- Port 8000<br/>- --reload mode"]
    ExpoLocal["Expo Dev Server<br/>- Metro Bundler<br/>- QR Code -> Expo Go"]
  end

  subgraph CloudInfra["Live Cloud Deployment (152.67.9.53)"]
    NginxCloud["Nginx Reverse Proxy<br/>- Port 80 (Public)<br/>- SSL Ready / Header Pass"]
    FastAPICloud["FastAPI + Uvicorn (abc-bank.service)<br/>- Port 8000 (Internal)<br/>- PBKDF2 and JWT Auth"]
    PostgresCloud[("PostgreSQL 14<br/>- Port 5432<br/>- Database: abc_bank")]
  end
  ...
  ExpoLocal -->|API Requests via EXPO_PUBLIC_API_URL| NginxCloud
...
```mermaid
flowchart LR
  Dev3["Developer"] -->|alembic revision --autogenerate| Migration["New Migration File<br/>(alembic/versions/)"]
  Migration -->|alembic upgrade head| LiveDB[("PostgreSQL<br/>(Live Schema)")]
  LiveDB -->|npm run db:introspect| PrismaFile["prisma/schema.prisma<br/>(Updated)"]
  PrismaFile -->|npm run db:studio| Studio["Prisma Studio<br/>(Visual Browser)"]
```
